...
def extract_data(**kwargs):
    """
    Tâche Airflow : Exécute le script de collecte des données via un sous-processus et enregistre les métriques.

    Paramètres
    ----------
    **kwargs : dict
        Dictionnaire contenant les objets contextuels d'Airflow (comme `ti` pour le TaskInstance).

    Comportement
    ------------
    - Lance un thread de monitoring de la consommation mémoire pendant l'exécution du script.
    - Exécute un script Python avec des arguments liés à la période d'extraction des tweets (10h30 - 10h30 J-1).
    ...
    # Définit l'heure d'aujourd'hui à 10h30
...
def insert_into_mongo(**kwargs):
    ...
    # Connexion à MongoDB via MongoHook d’Airflow (connexion définie dans Airflow)
    hook = MongoHook(mongo_conn_id='mongo_id_audensiel')
    db = hook.get_conn().Recolte  # Accès à la base Recolte

    # Insertion en masse des tweets nettoyés dans la collection 'tweets'
...
with DAG(
    dag_id="twitter_etl_daily_dag",               # Identifiant unique du DAG
    ...
    extract = PythonOperator(
        task_id="extract_data",                     # Nom de la tâche
        python_callable=extract_data                 # Fonction Python à exécuter
    )
    ...
    # Ordonnancement des tâches : extraction → transformation → insertion → calcul des échecs
